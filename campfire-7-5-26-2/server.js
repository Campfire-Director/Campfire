...
/* Live Reddit diagnostics: open this in a browser ON THE DEPLOYED SERVER
   and it runs the whole fetch pipeline fresh (no cache), reporting per
   subreddit exactly which layer worked or what error each host returned.
   Same optional STATS_KEY protection as /stats. */
app.get('/debug/reddit', async (req, res) => {
  if (process.env.STATS_KEY && req.query.key !== process.env.STATS_KEY) {
    return res.status(403).json({ error: 'wrong or missing ?key=' });
  }
  const report = {
    oauthConfigured: !!(process.env.REDDIT_CLIENT_ID && process.env.REDDIT_CLIENT_SECRET),
    cacheAgeSeconds: redditCache.at ? Math.round((Date.now() - redditCache.at) / 1000) : null,
    cachedItems: redditCache.items.length,
    subs: {},
  };
  if (report.oauthConfigured) {
    try { await getRedditToken(); report.oauthToken = 'ok'; }
    catch (e) { report.oauthToken = 'FAILED: ' + e.message; }
  }
  await Promise.all(REDDIT_SUBS.map(async (sub) => {
    const { posts, tried } = await fetchSubreddit(sub);
    report.subs[sub] = { got: posts.length, sample: posts[0] ? posts[0].text : null, tried };
  }));
  const totalPosts = Object.values(report.subs).reduce((a, s) => a + s.got, 0);
  report.verdict = totalPosts > 0
    ? `WORKING — ${totalPosts} posts fetched`
    : 'BROKEN — every layer failed; see per-sub errors above. If all say HTTP 403, set REDDIT_CLIENT_ID / REDDIT_CLIENT_SECRET (see README).';
  res.json(report);
});
...
function fetchUrl(url, opts = {}, redirectsLeft = 3) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.request(url, {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location && redirectsLeft > 0) {
        ...
        return resolve(fetchUrl(next, { ...opts, method: 'GET', body: undefined }, redirectsLeft - 1));
      }
      const chunks = [];
      res.on('data', c => chunks.push(c));
      res.on('end', () => {
        let buf = Buffer.concat(chunks);
        // Decompress if the CDN compressed the body. Without this, a
        // gzipped response looks like binary garbage, JSON.parse fails
        // inside a try/catch, and the failure is completely silent —
        // which is exactly the kind of bug that "just doesn't work."
        try {
          const enc = (res.headers['content-encoding'] || '').toLowerCase();
          if (enc.includes('gzip')) buf = zlib.gunzipSync(buf);
          else if (enc.includes('deflate')) buf = zlib.inflateSync(buf);
          else if (enc.includes('br') && zlib.brotliDecompressSync) buf = zlib.brotliDecompressSync(buf);
        } catch (e) { /* leave the raw body; the parser will complain visibly */ }
        const body = buf.toString('utf8');
        if (res.statusCode !== 200) {
          return reject(new Error('HTTP ' + res.statusCode + ' — ' + body.slice(0, 120).replace(/\s+/g, ' ')));
        }
        resolve(body);
      });
    });
...
/* ---------- Reddit access, three layers deep ----------
   Layer 1 (the reliable one): Reddit's official app-only OAuth. Reddit
   routinely 403-blocks unauthenticated JSON requests from datacenter
   IPs (Render, AWS, etc.) no matter what user-agent they wear — OAuth
   is the sanctioned road and works from anywhere. Setup takes 2 min:
     1. reddit.com/prefs/apps → "create app" → type: script
     2. On Render, set env vars REDDIT_CLIENT_ID and REDDIT_CLIENT_SECRET
   Layer 2: the public JSON endpoints (www + old), which work from
   residential IPs and sometimes from cloud ones.
   Layer 3: /debug/reddit — a live diagnostic page that runs the whole
   pipeline and reports exactly what each host returned, so a failure
...
async function getRedditToken() {
  ...
  const tokenUrl = (process.env.REDDIT_TOKEN_URL || 'https://www.reddit.com/api/v1/access_token');
  const body = await fetchUrl(tokenUrl, {
    method: 'POST',
    headers: {
      'Authorization': 'Basic ' + Buffer.from(id + ':' + secret).toString('base64'),
      'Content-Type': 'application/x-www-form-urlencoded',
      'User-Agent': 'web:campfire-party-game:v1.0 (by /u/campfire-host)',
    },
    body: 'grant_type=client_credentials',
  });
  const tok = JSON.parse(body);
  if (!tok.access_token) throw new Error('token response had no access_token');
  redditToken = { value: tok.access_token, expiresAt: Date.now() + Math.max(60, (tok.expires_in || 3600) - 300) * 1000 };
  return redditToken.value;
}

// Fetch one subreddit's hot posts, trying OAuth first, then public hosts.
// Returns { posts, tried: [{via, ok/error, count}] } so callers (and the
// debug endpoint) can see the whole story.
async function fetchSubreddit(sub) {
  ...
  try {
    ...
    if (token) {
      const oauthBase = process.env.REDDIT_OAUTH_URL || 'https://oauth.reddit.com';
      const body = await fetchUrl(oauthBase + '/r/' + sub + '/hot?limit=30&raw_json=1', {
        headers: {
      ...
      tried.push({ via: 'oauth', ok: true, count: posts.length, bodyStart: posts.length ? undefined : body.slice(0, 150) });
      if (posts.length) return { posts, tried };
    } else {
      tried.push({ via: 'oauth', skipped: 'REDDIT_CLIENT_ID / REDDIT_CLIENT_SECRET not set' });
    }
  } catch (e) {
    tried.push({ via: 'oauth', error: e.message });
  }
  // Layer 2: public JSON, www then old
  ...
  for (const host of hosts) {
    try {
      ...
      const posts = parseRedditPosts(body, sub);
      tried.push({ via: host, ok: true, count: posts.length, bodyStart: posts.length ? undefined : body.slice(0, 150) });
      if (posts.length) return { posts, tried };
    } catch (e) {
      tried.push({ via: host, error: e.message });
    }
  }
  return { posts: [], tried };
}
...
async function getRedditPosts() {
  ...
  await Promise.all(REDDIT_SUBS.map(async (sub) => {
    const { posts, tried } = await fetchSubreddit(sub);
    if (posts.length) all.push(...posts);
    else console.log('Reddit r/' + sub + ' yielded nothing:', JSON.stringify(tried));
  }));
...
function createRoom() {
  const room = {
    ...
    settings: { seconds: 90, minWords: 10, maxWords: 60, visibility: 'full', votingSeconds: 10, anonymous: false, directorTheme: true, mode: 'text', themeSource: 'words' },
