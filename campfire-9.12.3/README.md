...
- **Director's Theme** (a lobby setting): the Camp Director gets 45 seconds
  ...
  - **Theme inspiration** sub-setting: choose "Random words" (quirky short
    prompts) or "📰 News headlines" — the server pulls real current headlines
    from BBC and CNN public RSS feeds and offers ~5 for the Director to pick
    from. If the feeds are unreachable, it silently falls back to word
    prompts, so a game never stalls. Headlines are cached for 5 minutes.
    (Requires open outbound internet — works on Render; may be blocked in
