...
// Pick a handful of varied headlines for the Director to choose from
function pickHeadlines(all, n) {
  const pool = [...all];
  shuffle(pool);
  // Keep them a reasonable length for a story prompt
  return pool.filter(h => h.text.length >= 12 && h.text.length <= 120).slice(0, n);
}
...
function startTheming(room) {
  ...
  // If News Headline source is on, fetch in the background and push an update
  // when they arrive. The phase starts immediately with word suggestions so
  // the Director never waits on a slow (or failing) feed.
  if (room.settings.themeSource === 'news') {
    getHeadlines().then(all => {
      if (room.phase !== 'theming') return; // they already moved on
      room.themeHeadlines = pickHeadlines(all, 5);
      broadcast(room);
    }).catch(() => { /* feeds unavailable — word suggestions remain */ });
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('update_settings', (settings) => {
    ...
    if (settings.themeSource === 'words' || settings.themeSource === 'news') s.themeSource = settings.themeSource;
